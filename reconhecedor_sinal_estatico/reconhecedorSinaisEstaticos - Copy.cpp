...
int main(int argc, char** argv)
{
		...
		if (teclado == 'a' || teclado == 'b' || teclado == 'c') {
			if (teclado == 'a' && quantidadeTreinamentoA >= REPETICAO) {
				...
				if (teclado == 'a') {
					...
					label[quantidadeTreinamento] = SINAL_C;
	...
	if (TREINAMENTO == 1) {
		Mat labelTreinamento(QUANTIDADE, 1, CV_32FC1, label);
		...
		linearSVM.train_auto(dadosTreinamento, labelTreinamento, Mat(), Mat(), linearSVMparams, 20);
		linearSVM.save(buscarNomeArquivo(CvSVM::LINEAR));

		///treinamento polinomial quadrático
		polinomialSVMparams.degree	  = 2;//quadrática
		polinomialSVMparams.kernel_type = CvSVM::POLY;
		polinomialSVM.train_auto(dadosTreinamento, labelTreinamento, Mat(), Mat(), polinomialSVMparams, 20);
		polinomialSVM.save(buscarNomeArquivo(CvSVM::POLY));
		
		//treinamento radial
		CvParamGrid CvParamGrid_C(pow(2.0,-15), pow(2.0,15), pow(2.0,2));
		CvParamGrid CvParamGrid_gamma(pow(2.0,-15), pow(2.0,15), pow(2.0,2));
		radialSVMparams.kernel_type = CvSVM::RBF;
		radialSVM.train_auto(dadosTreinamento, labelTreinamento, Mat(), Mat(), radialSVMparams, 20, CvParamGrid_C, CvParamGrid_gamma);
